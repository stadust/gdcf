...
fn impl_from_raw_object(ast: &DeriveInput) -> Tokens {
            ...
            Fields::Named(ref fields) => {
                for field in fields.named.iter() {
                    ...
                    for meta_items in field.attrs.iter().filter_map(get_meta_items) {
                        for meta_item in meta_items {
                            match meta_item {
                                ...
                                NestedMeta::Meta(Meta::NameValue(ref nv))
                                    if nv.ident == "deserialize_with" =>
                                {
                                    if let Lit::Str(ref strlit) = nv.lit {
                                        if deserialize_with.is_some() {
                                            panic!(
                                                "'deserialize_with' set twice on field {}",
                                                field_name
                                            )
                                        }
                                        ...
                                        panic!(
                                            "'deserialize_with' not string value on field {}",
                                            field_name
                                        )
                                    }
                                }
                                    ...
                                    if nv.ident == "default" =>
                                {
                                    match default {
                                        DefaultValue::None => {
                                            default = DefaultValue::Literal(nv.lit.clone())
                                        }
                    ...
                    let mode = if flatten {
                        ...
                        let idx =
                            index.expect(&format!("no 'index' specified on field {}", field_name));
    ...
    let things = data.into_iter()
        .map(|FieldData { field_name, mode }| match mode {
