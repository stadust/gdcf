    ...
    C: Cache + 'static,
{
    ...
    request_maker: Box<dyn Fn(S) -> GdcfFuture<N, A::Err, C::Err> + Send + 'static>,
}
...
where
    ...
    C: Cache + 'static,
{
    pub fn new<F: 'static>(request: S, func: F) -> GdcfStream<S, N, A, C>
    where
        F: Fn(S) -> GdcfFuture<N, A::Err, C::Err> + Send,
    {
