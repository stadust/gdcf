...
pub mod to {
    ...
    ///
    /// # Errors
    /// If the decoded data cannot be put together as UTF8, an [`Utf8Error`] is returned
    ...
    /// Performs URL-safe base64 decoding on the given [`str`] and returns the decoded bytes
    ///
    /// # Errors
    /// If the given string isn't valid URL-safe base64, an [`DecodeError`] is returned
    pub fn b64_decoded_bytes(encoded: &str) -> Result<Vec<u8>, DecodeError> {
        base64::decode_config(encoded, URL_SAFE)
    }

    /// Performs URL-safe base64 decoding on the given [`str`] and tries to build a UTF8 String from
    /// the resulting bytes.
    ///
    /// # Errors
    /// If the given string isn't valid URL-safe base64, a [`DecodeError`] is returned
    ///
    /// # Panics
    ...
    /// Converts the given `u8` into a `bool` by returning `true` if `value != 0`, and `false` otherwise
    ///
    ...
    ///
    /// # Errors
    /// If the given string isn't `"0"` and also isn't valid URL-safe base64, a [`DecodeError`] is returned
    ...
    /// is done directly on the bytes, and the result of each byte-wise XOR operation is casted
    /// to `char`, meaning this function only works for ASCII strings.
...
pub mod from {
    ...
    /// Converts the given [`Vec`] of values convertible into signed integers into a robtop-approved
    /// string.
    ...
    /// Converts the given `bool` into an `u8`, returning `0` if `value == false` and `1`otherwise.
    ///
